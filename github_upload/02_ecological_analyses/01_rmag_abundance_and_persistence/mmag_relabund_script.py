...
FILTERED_HEADERS = [
    ...
    'contig_presence_ratio_at_filter', 'average_genome_wide_cov_at_filter'
]
...
def create_filtered_record(sample_name, state, month, caste, colony, rep,
    ...
    return {
        ...
        'average_genome_wide_cov_at_filter': average_genome_wide_cov_at_filter
    }
...
def process_single_bed_file(
    ...
    mag_coverage_data = defaultdict(lambda: {'coverage_segments': [], 'total_bases_in_bed': 0})
            ...
            for line_num, line in enumerate(bed_in, 1):
                ...
                if chrom not in contig_to_metadata_map:
                    filtered_contig_records_for_sample.append(create_filtered_record(
                        ...
                        "contig_not_in_renamed", 'N/A', 0, 0, 0.0, 0.0
                    ))
                ...
                if mag_id is None:
                    ...
                    filtered_contig_records_for_sample.append(create_filtered_record(
                        ...
                        "contig_has_no_MAG_id", 'N/A', 0, 0, 0.0, 0.0
                    ))
                ...
                mag_coverage_data[mag_id]['total_bases_in_bed'] += contig_length_in_bed_segment
    ...
    for mag_id, observed_contigs_set in mag_observed_contigs.items():
        ...
        average_genome_wide_cov = 0.0
        current_mag_cov_segments = mag_coverage_data[mag_id]['coverage_segments']
        
        # Calculate total reads for the MAG (sum of coverage * length for each segment)
        total_reads_for_mag = sum(seg[0] * seg[1] for seg in current_mag_cov_segments)

        # Calculate average genome-wide coverage for the MAG
        if mag_genome_size is not None and mag_genome_size > 0:
            average_genome_wide_cov = total_reads_for_mag / mag_genome_size

        filter_reason = None
        filter_value = None

        # Filter 1: Genome Size (only filter remaining)
        ...
        if filter_reason:
            ...
            for chrom in observed_contigs_set:
                ...
                filtered_contig_records_for_sample.append(create_filtered_record(
                    ...
                    current_contig_presence_ratio, average_genome_wide_cov
                ))
        else:
            ...
            passed_mags_data[mag_id] = {
                ...
                'average_genome_wide_cov': average_genome_wide_cov
            }
    ...
    for mag_id, mag_metrics in passed_mags_data.items():
        for chrom in mag_observed_contigs[mag_id]: # Iterate over only observed contigs that passed
            ...
            record = {
                ...
                'average_genome_wide_cov': mag_metrics['average_genome_wide_cov']
            }
