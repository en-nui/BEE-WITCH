...
library(corrplot)
library(pheatmap)
ecology_data <- read.csv('/home/robinch/projects/BEE-WITCH/16b_rMAG_mapping/ecology_analyses/ecology_metadata_df.csv')
...
# --- 2. Aggregate All Nucleotide Diversity Files ---
...
# b. Get a list of all gene-level summary files
file_list <- list.files(
  path = popgen_dir,
  pattern = "_filtered_snps_by_gene\\.csv$", 
  full.names = TRUE
)

# c. Read and concatenate all files into one dataframe
nuc_div_df <- purrr::map_dfr(file_list, read.csv)

# d. Rename sample_id to match your ecology data
nuc_div_df <- nuc_div_df %>%
  rename(sample_name = sample_id)

cat("--- Nucleotide Diversity Data Loaded ---\n")
cat("Loaded", length(file_list), "files with a total of", nrow(nuc_div_df), "rows.\n")





# --- 1. Calculate Mean Nucleotide Diversity per Colony ---
# (Requires the 'nuc_div_df' from above)
mean_nuc_div_per_colony <- nuc_div_df %>%
  # We need the 'Colony' information for each sample
  left_join(select(ecology_data, sample_name, Colony) %>% distinct(), by = "sample_name") %>%
  group_by(Colony) %>%
  # Calculate the overall average diversity for the entire colony over time
  summarise(mean_nucleotide_diversity = mean(nucDiv_per_bp, na.rm = TRUE))

# --- 2. Combine with Temporal Decay Results ---
# (Requires 'all_community_mantel_results' from your script)
microbial_stability <- all_community_mantel_results$Microbial

# Join the two datasets
stability_vs_genetics_df <- inner_join(microbial_stability, mean_nuc_div_per_colony, by = "Colony")

# --- 3. Test and Plot the Relationship ---
# Run a Spearman correlation
correlation_test <- cor.test(
  ~ Mantel_r + mean_nucleotide_diversity, 
  data = stability_vs_genetics_df,
  method = "spearman"
)

cat("\n--- Correlation: Community Stability vs. Genetic Diversity ---\n")
print(correlation_test)

# Plot the relationship
ggplot(stability_vs_genetics_df, aes(x = mean_nucleotide_diversity, y = Mantel_r)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(
    title = "Community Stability vs. Average Genetic Diversity",
    x = "Mean Nucleotide Diversity (within MAGs)",
    y = "Community Stability (Temporal Mantel's R)"
  ) +
  theme_bw()
...
# --- 1. Create a Shannon Diversity dataframe that INCLUDES Colony ---
shannon_df <- ecology_data %>%
  filter(type == "mmag") %>%
  group_by(sample_name) %>%
  summarise(
    Shannon = vegan::diversity(mean, index = "shannon"),
    Colony = first(Colony), # FIX: Add this line to keep the Colony variable
    .groups = 'drop'
  )

# --- 2. Set Up the Bootstrap ---
# (Assuming 'gamm_data' is your full, prepared dataframe)
all_mags <- unique(gamm_data$MAG_id)
n_iterations <- 5 
n_mags_per_sample <- 10 

bootstrap_summary_results <- list()
bootstrap_plot_data <- list()

shannon_seq <- seq(min(gamm_data$Shannon), max(gamm_data$Shannon), length.out = 100)
...
for (i in 1:n_iterations) {
  ...
  set.seed(i)
  sampled_mags <- sample(all_mags, n_mags_per_sample)
  
  gamm_data_subset <- gamm_data %>%
    filter(MAG_id %in% sampled_mags) %>%
    mutate(MAG_id = as.factor(MAG_id), Colony = as.factor(Colony))
  
  model_fit <- gam(
    nucDiv_per_bp ~ s(Shannon) + s(average_coverage) + s(Colony, MAG_id, bs = "re"),
    data = gamm_data_subset,
    method = "REML"
  )
  
  model_summary <- summary(model_fit)
  shannon_term <- model_summary$s.table["s(Shannon)", ]
  bootstrap_summary_results[[i]] <- data.frame(
    iteration = i,
    shannon_p_value = shannon_term[["p-value"]]
  )
  
  # FIX: Add dummy variables for all random effects to the prediction dataframe
  pred_df <- data.frame(
    Shannon = shannon_seq, 
    average_coverage = mean(gamm_data_subset$average_coverage),
    Colony = gamm_data_subset$Colony[1], # Use the first colony as a dummy
    MAG_id = sampled_mags[1] # Use the first MAG as a dummy
  )
  
  predictions <- predict(model_fit, newdata = pred_df, se.fit = TRUE)
  
  bootstrap_plot_data[[i]] <- data.frame(
    iteration = i,
    Shannon = shannon_seq,
    fit = predictions$fit,
...
# a. Significance: What proportion of runs were significant?
final_bootstrap_summary <- bind_rows(bootstrap_summary_results)
proportion_significant <- final_bootstrap_summary %>%
  summarise(proportion_sig = sum(shannon_p_value < 0.05) / n())

cat("\n--- Bootstrap Significance Summary ---\n")
print(proportion_significant)
hist(final_bootstrap_summary$shannon_p_value, main = "Distribution of p-values for s(Shannon)")


# b. Shape of the Relationship: Plot the mean smooth curve
final_plot_data <- bind_rows(bootstrap_plot_data)
mean_smooth_data <- final_plot_data %>%
  group_by(Shannon) %>%
  summarise(
    mean_fit = mean(fit),
    mean_se = mean(se),
  ...
  mutate(
    conf_low = mean_fit - 2 * mean_se,
    conf_high = mean_fit + 2 * mean_se
  )

ggplot(mean_smooth_data, aes(x = Shannon, y = mean_fit)) +
  geom_ribbon(aes(ymin = conf_low, ymax = conf_high), alpha = 0.2, fill = "purple") +
  geom_line(color = "purple", linewidth = 1) +
  labs(
    title = "Mean Effect of Shannon Diversity Across Bootstrap Runs",
    x = "Shannon Diversity",
    y = "Centered Effect on Nucleotide Diversity"
  ) +
  theme_bw()
...
# --- 2. Assign Final Class to Each MAG and Calculate Proportions ---
# (Assuming 'classified_genera_gmm' and 'genus_summary' are loaded)
mag_stability_proportions <- classified_genera_gmm %>%
  # Join the final 'Mixed' classification to each MAG
  left_join(select(genus_summary, Genus, Final_Class), by = "Genus") %>%
  # Count the number of MAGs in each Final_Class for each Colony
  count(Colony, Final_Class) %>%
...
cat("--- Proportion of MAGs by Stability Class per Colony ---\n")
print(as.data.frame(mag_stability_proportions))

# --- 4. (Optional) Create a Plot ---
ggplot(mag_stability_proportions, aes(x = as.factor(Colony), y = Proportion, fill = Final_Class)) +
  geom_bar(stat = "identity", position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values = c("Stable" = "#1f78b4", "Dynamic" = "#e31a1c", "Mixed" = "#b2df8a")) +
  labs(
    title = "Proportion of MAGs by Genus Stability Class Across Colonies",
    x = "Colony",
    y = "Proportion of MAGs",
    fill = "Genus Stability Class"
  ) +
  theme_bw() +
...
# --- 2. Calculate Proportions of MAG Stability Classes per Colony ---
# (Assuming 'classified_genera_gmm' is loaded)
mag_proportions <- classified_genera_gmm %>%
  # Count the number of MAGs in each stability class for each colony
  count(Colony, Stability_Class) %>%
  # Calculate the proportion
  group_by(Colony) %>%
  mutate(Proportion = n / sum(n)) %>%
  ungroup()
...
# --- 3. Create the Plot ---
ggplot(mag_proportions, aes(x = as.factor(Colony), y = Proportion, fill = Stability_Class)) +
  geom_bar(stat = "identity", position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values = c("Stable" = "#1f78b4", "Dynamic" = "#e31a1c")) +
  labs(
    title = "Proportion of MAG Stability Classes Across Colonies",
    x = "Colony",
    y = "Proportion of MAGs",
    fill = "MAG Stability Class"
  ) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# ============================================================
# FIGURE 2 WORKFLOW:
# ECOLOGICAL PERSISTENCE OF ALL BACTERIAL MAGs
#
# Starts from contig-level ecology_data.
#
# Includes:
#   1. All bacterial MAGs
#   2. Core-associated versus other genus annotation
#   3. Contig-to-sample/MAG collapse
#   4. Explicit sample x MAG nondetections
#   5. Abundance floor and log2 transformation
#   6. Replicate-level detection
#   7. Longitudinal occupancy
#   8. Persistent/intermittent classification
#   9. Replicate-corrected temporal abundance variance
#  10. Scatter plot with marginal densities
#  11. Summary tables and output files
#
...
# Predefined genera that are stably associated with the
...
# Ecological detection thresholds.
#
# Replace these with the exact thresholds used in the final
# common-database mapping analysis, if they differ.
MIN_CONTIG_PRESENCE <- 0.25
MIN_MEAN_ABUNDANCE <- 1

# A MAG is considered detected in a colony-month when it is
# detected in at least this fraction of valid replicate pools.
MIN_REPLICATE_DETECTION_FRACTION <- 0.50

# Longitudinal persistence threshold.
PERSISTENT_MIN_OCCUPANCY <- 0.75

# Minimum number of eligible colony-months needed to classify
# a trajectory as persistent or intermittent.
MIN_ELIGIBLE_MONTHS <- 4

# Minimum number of detected months required to estimate
# temporal abundance variance.
MIN_MONTHS_FOR_VARIANCE <- 3


# ------------------------------------------------------------
# 2. Select all bacterial worker-associated MAG records
#
# IMPORTANT:
# No genus filtering occurs here.
# ------------------------------------------------------------

ecology_worker_contigs_all <- ecology_data %>%
  filter(
    type == "mmag",
    Caste == "Worker"
  ) %>%
...
if (nrow(ecology_worker_contigs_all) == 0) {
  stop(
    "No worker-associated bacterial MAG records were found."
  )
}

cat("\n--- Initial all-MAG input ---\n")
cat(
  "Contig-level rows:",
  nrow(ecology_worker_contigs_all),
  "\n"
)
cat(
  "Unique bacterial MAGs:",
  n_distinct(ecology_worker_contigs_all$MAG_id),
  "\n"
)
cat(
  "Unique genera:",
  n_distinct(ecology_worker_contigs_all$Genus),
  "\n"
)


# ------------------------------------------------------------
# 3. Validate the contig-level structure
#
# MAG-level abundance and QC values should be identical across
# the contig rows belonging to one sample x MAG combination.
# ------------------------------------------------------------

contig_structure_check <- ecology_worker_contigs_all %>%
  group_by(
    sample_name,
    MAG_id
  ) %>%
  summarise(
    ...
    n_unique_genome_size =
      n_distinct(genome_size),
    
    n_unique_total_contigs =
      n_distinct(
        total_contigs_in_MAG_in_sheet
      ),
    
    n_unique_observed_contigs =
      n_distinct(
        observed_contigs_in_bed_file
      ),
    
    n_unique_presence_ratio =
      n_distinct(
        contig_presence_ratio
      ),
    
    .groups = "drop"
  )

cat("\n--- Contig-level validation ---\n")
cat(
  "Sample x MAG combinations:",
  nrow(contig_structure_check),
  "\n"
)
cat(
  "Groups containing multiple abundance values:",
  sum(contig_structure_check$n_unique_mean > 1),
  "\n"
)
cat(
  "Groups containing multiple presence-ratio values:",
  sum(
    contig_structure_check$n_unique_presence_ratio > 1
  ),
  "\n"
)

# Stop rather than silently choosing one abundance value if
# sample x MAG groups contain multiple abundance estimates.
if (any(contig_structure_check$n_unique_mean > 1)) {
  
  problematic_abundance_groups <-
    contig_structure_check %>%
    filter(n_unique_mean > 1)
  ...
  stop(
    paste(
      "Some sample x MAG combinations contain multiple",
      "values of `mean`. Inspect these before proceeding."
    )
  )
}
...
# ------------------------------------------------------------
# 4. Collapse from contig rows to one row per sample x MAG
#
# The MAG-level values are repeated across contigs, so use
# first(), not sum().
# ------------------------------------------------------------

mag_sample_observed_all <- ecology_worker_contigs_all %>%
  group_by(
    sample_name,
    State,
    Month,
...
# Confirm one row per sample x MAG.

sample_mag_duplicates <- mag_sample_observed_all %>%
  dplyr::count(
    sample_name,
    MAG_id,
    name = "n_rows"
...
if (nrow(sample_mag_duplicates) > 0) {
  
  print(
    head(
      sample_mag_duplicates,
      20
    )
  )
  ...
  stop(
    paste(
      "mag_sample_observed_all is not unique by",
      "sample_name x MAG_id."
    )
  )
}
...
# ------------------------------------------------------------
# 5. Annotate core-associated versus other genera
# ------------------------------------------------------------

mag_sample_observed_all <-
  mag_sample_observed_all %>%
  mutate(
    genus_association = if_else(
      Genus %in% target_genera,
      "Core-associated genus",
      "Other genus"
    ),
    
    genus_association = factor(
      genus_association,
      levels = c(
        "Core-associated genus",
        "Other genus"
...
cat("\n--- MAGs by genus-association category ---\n")
...
cat("\n--- Genera included in each category ---\n")
...
# ------------------------------------------------------------
# 6. Define replicate-level ecological detection
#
# Replace this rule with the exact final mapping criteria if
# the remapped table contains a dedicated passes_detection
# column or additional breadth/coverage filters.
# ------------------------------------------------------------
...
cat("\n--- Replicate-level detection summary ---\n")
...
# ------------------------------------------------------------
# 7. Define the complete set of valid worker libraries
#
# This determines the eligible replicate and month denominator.
#
# A separate validated sample metadata table would be even
# better, but this is valid when every retained worker library
# occurs somewhere in ecology_data.
# ------------------------------------------------------------

sample_grid_all <- ecology_worker_contigs_all %>%
  distinct(
    sample_name,
    State,
    Month,
...
sample_name_duplicates <- sample_grid_all %>%
  dplyr::count(
    sample_name,
    name = "n_rows"
  ) %>%
...
if (nrow(sample_name_duplicates) > 0) {
  
  print(
    head(
      sample_name_duplicates,
      20
    )
  )
  ...
  stop(
    paste(
      "Some sample names correspond to multiple",
...
# ------------------------------------------------------------
# 8. Define all observed MAG x colony populations
#
# A MAG-colony population enters the analysis when it passes
# detection in at least one valid replicate library.
# ------------------------------------------------------------

mag_colony_pairs_all <-
  mag_sample_observed_all %>%
  filter(detected) %>%
  distinct(
    Colony,
...
cat("\n--- All detected MAG-colony populations ---\n")
cat(
  "MAG-colony populations:",
  nrow(mag_colony_pairs_all),
  "\n"
)
cat(
  "Unique MAGs:",
  n_distinct(mag_colony_pairs_all$MAG_id),
  "\n"
)
...
print(
  mag_colony_pairs_all %>%
    dplyr::count(
      genus_association,
      name = "n_mag_colony_populations"
    )
)


# ------------------------------------------------------------
# 9. Expand each MAG-colony population over every eligible
#    replicate library sampled from that colony
#
# Missing combinations become explicit nondetections.
# ------------------------------------------------------------

mag_sample_complete_all <-
  mag_colony_pairs_all %>%
  inner_join(
    sample_grid_all,
    by = "Colony",
...
# Check uniqueness after completing the grid.

completed_grid_duplicates <-
  mag_sample_complete_all %>%
  dplyr::count(
    sample_name,
    MAG_id,
...
if (nrow(completed_grid_duplicates) > 0) {
  ...
  stop(
    paste(
      "Duplicate sample x MAG rows remain after",
      "completion of the sample grid."
    )
  )
}
...
# ------------------------------------------------------------
# 10. Calculate the abundance floor
#
# Use half the smallest positive abundance among observations
# that pass the ecological detection criteria.
#
# The floor is used only for transformation. The untransformed
# abundance remains zero for nondetections.
# ------------------------------------------------------------

positive_detected_abundances <-
  mag_sample_complete_all %>%
  filter(
    detected,
    abundance > 0
...
if (length(positive_detected_abundances) == 0) {
  stop(
    paste(
      "No positive detected abundance values were",
      "available for calculating the abundance floor."
    )
  )
}


smallest_positive_abundance <-
  min(
    positive_detected_abundances,
    na.rm = TRUE
  )
...
abundance_floor <-
  smallest_positive_abundance / 2


if (
  !is.finite(abundance_floor) ||
  abundance_floor <= 0
) {
  stop(
    "The calculated abundance floor is invalid."
  )
}
...
cat(
  "Smallest positive detected abundance:",
  smallest_positive_abundance,
  "\n"
)
cat(
...
# ------------------------------------------------------------
# 11. Log2-transform replicate-level abundance
#
# log2(A + abundance_floor)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 12. Summarize biological replicate pools by colony-month
#
# n_detected_replicates counts unique sample names, not contigs.
# ------------------------------------------------------------

mag_month_summary_all <-
  mag_sample_complete_all %>%
  group_by(
    Colony,
    MAG_id,
  ...
  summarise(
    ...
    n_detected_replicates =
      n_distinct(
        sample_name[detected]
      ),
    ...
    # Mean of replicate-level log2 abundance values.
    mean_log2_abundance =
      mean(
        log2_abundance,
        na.rm = TRUE
  ...
  mutate(
    detected_any =
      n_detected_replicates >= 1,
    
    detected_majority =
      detection_fraction >=
      MIN_REPLICATE_DETECTION_FRACTION,
...
# Validate monthly replicate counts.

invalid_monthly_detection <-
  mag_month_summary_all %>%
  filter(
    n_detected_replicates >
      n_replicates |
      detection_fraction < 0 |
      detection_fraction > 1
  )

if (nrow(invalid_monthly_detection) > 0) {
  ...
  stop(
    paste(
      "Detected replicate counts exceed eligible",
      "replicate counts."
    )
  )
}
...
cat("\n--- Colony-month replicate detection counts ---\n")
...
# ------------------------------------------------------------
# 13. Helper function for consecutive detection runs
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 14. Calculate longitudinal occupancy and persistence
# ------------------------------------------------------------
...
mag_persistence_all <-
  ...
  mutate(
    ...
    ecologically_persistent =
      sufficiently_observed &
      persistence_fraction >=
      PERSISTENT_MIN_OCCUPANCY &
      detected_early &
      detected_late,
    
    occupancy_class = case_when(
      !sufficiently_observed ~
...
# ------------------------------------------------------------
# 15. Calculate replicate-corrected temporal variance
#
# Only colony-months meeting the replicate detection threshold
# contribute to temporal abundance-variance estimation.
#
# Corrected variance:
#
# total variance among monthly means
# minus
# mean(replicate variance / replicate count)
# ------------------------------------------------------------

mag_variance_components_all <-
  mag_month_summary_all %>%
  filter(
    month_detected
  ) %>%
  ...
  summarise(
    n_months_for_variance =
      n(),
    
    total_temporal_variance =
      if_else(
        n_months_for_variance >=
          MIN_MONTHS_FOR_VARIANCE,
  ...
  mutate(
    ...
    corrected_temporal_sd =
      sqrt(
        corrected_temporal_variance
      ),
    
    temporal_signal_fraction =
      case_when(
        is.na(
          total_temporal_variance
        ) ~
...
# ------------------------------------------------------------
# 16. Create final all-MAG population-level table
# ------------------------------------------------------------

mag_ecological_persistence_all <-
  mag_persistence_all %>%
  left_join(
    mag_variance_components_all,
...
# ------------------------------------------------------------
# 17. Validate representation of both genus categories
# ------------------------------------------------------------
...
cat("\n--- Core versus other validation ---\n")
print(association_validation)


if (
  association_validation$n_other_populations == 0
) {
  warning(
    paste(
      "No sufficiently observed populations were assigned",
      "to `Other genus`. Confirm that the upstream table",
      "was not previously genus-filtered."
    )
  )
}
...
# ------------------------------------------------------------
# 18. Create figure datasets
#
# Top occupancy distribution:
#   all sufficiently observed populations
#
# Main scatter and right density:
#   populations with estimable temporal variance
# ------------------------------------------------------------

occupancy_density_data_all <-
  mag_ecological_persistence_all %>%
  filter(
    sufficiently_observed,
    !is.na(
...
# ------------------------------------------------------------
# 19. Summary counts before plotting
# ------------------------------------------------------------
...
cat("\n--- Persistence classes by genus association ---\n")
print(class_summary_all)
...
cat("\n--- Variance-estimable populations ---\n")
print(scatter_summary_all)


# ------------------------------------------------------------
# 20. Define plot colors
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 21. Main coordinate plot
# ------------------------------------------------------------

set.seed(123)

main_plot_all <- ggplot(
  scatter_data_all,
  aes(
    x = persistence_fraction,
    y = corrected_temporal_sd
  )
) +
  annotate(
    "rect",
  ...
  labs(
    ...
    y =
      paste0(
        "Replicate-corrected temporal SD\n",
        "(log2 abundance)"
      ),
    ...
    caption =
      paste0(
        "*Persistent populations were detected in at least ",
        "75% of eligible months and near both ends of the ",
        "colony time series."
      )
  ) +
  theme_bw() +
...
# ------------------------------------------------------------
# 22. Top marginal occupancy densities
#
# Includes every sufficiently observed population.
# ------------------------------------------------------------

top_density_all <- ggplot(
  occupancy_density_data_all,
  aes(
    x =
      persistence_fraction,
...
# ------------------------------------------------------------
# 23. Right marginal temporal-variance densities
#
# Includes only variance-estimable populations.
# ------------------------------------------------------------

right_density_all <- ggplot(
  scatter_data_all,
  aes(
    x =
      corrected_temporal_sd,
...
# ------------------------------------------------------------
# 24. Assemble the final figure
# ------------------------------------------------------------

figure2_all_MAG_persistence <-
  (
    top_density_all |
      plot_spacer()
  ) /
...
# ------------------------------------------------------------
# 25. Overall summaries by genus association
# ------------------------------------------------------------

persistence_results_by_association <-
  occupancy_density_data_all %>%
  group_by(
    genus_association
  ) %>%
  summarise(
    n_evaluable_populations =
      n(),
    ...
    proportion_complete_occupancy =
      mean(
        persistence_fraction == 1,
        na.rm = TRUE
      ),
...
cat("\n--- Persistence by genus association ---\n")
print(
  persistence_results_by_association
)
...
# ------------------------------------------------------------
# 26. Temporal-variance summaries by genus association
# ------------------------------------------------------------

variance_results_by_association <-
  scatter_data_all %>%
  group_by(
    genus_association
  ) %>%
  summarise(
    n_variance_estimable =
      n(),
    
    median_corrected_temporal_sd =
      median(
        corrected_temporal_sd,
        na.rm = TRUE
      ),
    
    corrected_temporal_sd_q25 =
      quantile(
        corrected_temporal_sd,
        0.25,
        na.rm = TRUE
    ...
    proportion_zero_corrected_variance =
      mean(
        corrected_temporal_sd == 0,
        na.rm = TRUE
      ),
...
cat("\n--- Temporal variance by genus association ---\n")
print(
  variance_results_by_association
)
...
# ------------------------------------------------------------
# 27. Full genus-level summary
# ------------------------------------------------------------

persistence_results_by_genus <-
  occupancy_density_data_all %>%
  group_by(
    genus_association,
    Genus
...
# ------------------------------------------------------------
# 28. Optional descriptive distribution comparisons
#
# These do not account for repeated MAGs among colonies.
# Treat them as descriptive/supplementary comparisons unless
# replaced with an appropriate mixed-effects model.
# ------------------------------------------------------------

if (
  n_distinct(
    occupancy_density_data_all$genus_association
  ) == 2
) {
  ...
  cat("\n--- Wilcoxon test: longitudinal occupancy ---\n")
  print(
    occupancy_wilcox
  )
}
...
if (
  n_distinct(
    scatter_data_all$genus_association
  ) == 2
) {
  ...
  cat("\n--- Wilcoxon test: corrected temporal SD ---\n")
  print(
    variance_wilcox
  )
}
...
# ------------------------------------------------------------
# 29. Save analysis outputs
# ------------------------------------------------------------

write.csv(
  mag_sample_observed_all,
  "all_MAG_sample_level_collapsed.csv",
  row.names = FALSE
)

write.csv(
  mag_sample_complete_all,
  "all_MAG_sample_level_complete_grid.csv",
  row.names = FALSE
)

write.csv(
  mag_month_summary_all,
  "all_MAG_colony_month_summary.csv",
  row.names = FALSE
)

write.csv(
  mag_ecological_persistence_all,
  "all_MAG_ecological_persistence_population_level.csv",
  row.names = FALSE
)

write.csv(
  persistence_results_by_association,
  "all_MAG_persistence_by_genus_association.csv",
  row.names = FALSE
)

write.csv(
  variance_results_by_association,
  "all_MAG_variance_by_genus_association.csv",
  row.names = FALSE
)

write.csv(
  persistence_results_by_genus,
  "all_MAG_persistence_by_genus.csv",
  row.names = FALSE
)
...
ggsave(
  filename =
    "Figure2_all_MAG_persistence_core_association.pdf",
...
persistent_other_populations <- occupancy_density_data_all %>%
  filter(
    genus_association == "Other genus",
    occupancy_class == "Persistent"
  ) %>%
  select(
    Colony,
    MAG_id,
...
observation_summary <- occupancy_density_data_all %>%
  group_by(genus_association) %>%
  summarise(
    n_populations = n(),
    
    median_eligible_months =
      median(n_eligible_months),
    
    eligible_months_q25 =
      quantile(n_eligible_months, 0.25),
    
    eligible_months_q75 =
      quantile(n_eligible_months, 0.75),
    
    median_detected_months =
      median(n_detected_months),
    
    detected_months_q25 =
      quantile(n_detected_months, 0.25),
    
    detected_months_q75 =
      quantile(n_detected_months, 0.75),
...
persistence_by_trajectory_length <- occupancy_density_data_all %>%
  dplyr::count(
    genus_association,
    n_eligible_months,
    occupancy_class,
...
print(
  persistence_by_trajectory_length,
  n = Inf
)



population_abundance_summary <- mag_month_summary_all %>%
  group_by(
    Colony,
    MAG_id,
    Genus,
  ...
  summarise(
    median_monthly_abundance =
      median(
        mean_abundance[month_detected],
        na.rm = TRUE
      ),
    
    mean_monthly_abundance =
      mean(
        mean_abundance[month_detected],
        na.rm = TRUE
      ),
    
    maximum_monthly_abundance =
      max(
        mean_abundance,
        na.rm = TRUE
      ),
  ...
  mutate(
    median_monthly_abundance =
      if_else(
        is.nan(median_monthly_abundance),
        NA_real_,
        median_monthly_abundance
      )
  )
...
persistence_with_abundance <- occupancy_density_data_all %>%
  left_join(
    population_abundance_summary,
    by = c(
      "Colony",
      "MAG_id",
      "Genus",
...
persistence_with_abundance %>%
  group_by(genus_association) %>%
  summarise(
    median_population_abundance =
      median(
        median_monthly_abundance,
        na.rm = TRUE
      ),
...
persistence_table <- occupancy_density_data_all %>%
  dplyr::count(
    genus_association,
    occupancy_class
  ) %>%
  tidyr::pivot_wider(
    names_from = occupancy_class,
    values_from = n,
    values_fill = 0
  )
...
persistence_matrix <- occupancy_density_data_all %>%
  mutate(
    persistent =
      occupancy_class == "Persistent"
  ) %>%
...
fisher_persistence <- fisher.test(
  persistence_matrix
)

print(fisher_persistence)
...
data.frame(
  odds_ratio =
    unname(fisher_persistence$estimate),
  
  confidence_low =
    fisher_persistence$conf.int[1],
  
  confidence_high =
    fisher_persistence$conf.int[2],
  
  p_value =
    fisher_persistence$p.value
)
...
persistence_model_data <- persistence_with_abundance %>%
  mutate(
    persistent =
      occupancy_class == "Persistent",
    
    log_median_abundance =
      log2(
        median_monthly_abundance +
          abundance_floor
      ),
...
persistence_model_adjusted <- glmer(
  persistent ~
    genus_association +
    scale(log_median_abundance) +
    n_eligible_months +
    (1 | Colony) +
    (1 | MAG_id),
  
  data =
    persistence_model_data,
...
variance_eligibility_summary <- occupancy_density_data_all %>%
  dplyr::count(
    genus_association,
    name = "n_evaluable"
  ) %>%
  left_join(
    scatter_data_all %>%
      dplyr::count(
        genus_association,
        name = "n_variance_estimable"
      ),
    by = "genus_association"
  ) %>%
  mutate(
    proportion_variance_estimable =
      n_variance_estimable /
      n_evaluable
  )

print(variance_eligibility_summary)


ecological_summary <- occupancy_density_data_all %>%
  mutate(
    variance_estimable =
      !is.na(corrected_temporal_sd) &
      n_months_for_variance >=
      MIN_MONTHS_FOR_VARIANCE
  ) %>%
  dplyr::count(
